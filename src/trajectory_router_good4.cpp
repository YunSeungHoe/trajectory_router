...
public:
  TrajectoryRouter() : Node("trajectory_router")
  {
    ...
    primitive2DVector_0 = setPrimitiveVector(primitive, 4); // 차선의 수 (yaml에  넣어야 함 )
    ...
    primitive2DVector_2 = setPrimitiveVector(primitive, 4); // 차선의 수 (yaml에  넣어야 함 )
    primitive = this->get_parameter("primitive_3").as_integer_array();
    primitive2DVector_3 = setPrimitiveVector(primitive, 4); // 차선의 수 (yaml에  넣어야 함 )
    primitive = this->get_parameter("primitive_4").as_integer_array();
    primitive2DVector_4 = setPrimitiveVector(primitive, 4); // 차선의 수 (yaml에  넣어야 함 )
    primitive = this->get_parameter("primitive_5").as_integer_array();
    primitive2DVector_5 = setPrimitiveVector(primitive, 4); // 차선의 수 (yaml에  넣어야 함 )
...
private:
  ...
  void callbackPose(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
  {
    ...
    // topic 채우는 부분 
    laneletState = false; // 해당 라인은 지울 것 아래 라인이 실행되지 않도록 함
    if(laneletState){
